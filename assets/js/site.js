...
const data = {
  proj: {
    ...
    p4: {
      ...
      `,
      tags: ['Python', 'MATLAB', 'scikit-learn', 'Signal Processing', 'Machine Learning'],
      links: [
    ...
    p7: {
      ...
      `,
      tags: ['C', 'Python', 'Embedded Systems', 'Computer Vision', 'SPI']
    },
    p8: {
    ...
    p10: {
      ...
      htmlShort: `
        <ul style="font-size:0.9rem;line-height:1.8;color:var(--muted);padding-left:1.2rem">
          ...
          <li>Led path-following logic implementation, TCP (Transmission Control Protocol) server-client architecture, and PID (Proportional-Integral-Derivative) controller configuration for real-world robot operation</li>
          <li>Owned the testing effort — designed the QA test plan, test scenarios, simulation field, and testing matrix; ran six successful trials across varying levels of map complexity</li>
          <li>Led technical documentation including a video walkthrough of the system</li>
        </ul>
      `,
      html: `
        ...
        `,
      tags: ['C++', 'Multi-Agent Systems', 'Path Planning', 'Robotics', 'PID Control', 'TCP/IP'],
      links: [
    ...
    p11: {
      ...
      html: `
        ...
        `,
      tags: ['C++', 'ROS 2', 'Robotics', 'Motion Control'],
      links: [
    ...
    p12: {
      ...
      htmlShort: `
        <ul style="font-size:0.9rem;line-height:1.8;color:var(--muted);padding-left:1.2rem">
          <li>Identified a gap in a NYC recreational soccer league's player management setup firsthand as a team captain — attendance over WhatsApp, schedules over email, enrollment through a Google Sheet — and decided to build an app to address it</li>
          ...
          <li>Wrote documentation covering system requirements, build and run instructions, and full app functionality specs</li>
        </ul>
      `,
      html: `
        ...
        `,
      tags: ['React Native', 'JavaScript', 'iOS', 'Full-Stack Development'],
      links: [
  ...
  expMeta: {
    ...
    e7: { jobTitle: 'Product Engineer', company: 'Basis AI', location: 'New York, NY', dates: 'December 2025 — March 2026', tagline: 'Technical owner of full-stack development for key product features from design to production at an early stage start-up.', website: 'https://www.getbasis.ai/' }
  },
  exp: {
    e7: {
      htmlShort: `
        <ul style="font-size:0.9rem;line-height:1.8;color:var(--muted);padding-left:1.2rem">
          <li>Stepped in as de facto technical owner of the reconciliations product area within two months when the previous lead departed — the only engineer actively working on one of the most critical parts of the product</li>
          ...
          <li>Inherited the Reconciliation Editor — an improved UX replacement for what was in production — diagnosed and resolved blocking issues across frontend and backend logic, led QA, and drove it to production</li>
      ...
      `,
      html: `
        <p>Basis AI is an early-stage, AI-native accounting automation platform. The company has an entire internal team dedicated to making every employee more productive with AI, treating company context with the same rigor as code: version control, ownership, and deployment pipelines for internal knowledge. Working there meant genuinely depending on AI tooling at every stage — using AI agents in a dedicated planning step (where the agent reasons through the problem before acting) to understand system logic before touching code, and using MCP servers (tool integrations that extend what an AI agent can access) to pull in logging and traceability data and reproduce failures before implementing fixes. Maintaining a CLAUDE.md — a file that gives AI agents the context they need to work effectively within a codebase — and consistent coding standards made it possible to move quickly without sacrificing quality.</p>
        <p>The role also required picking up an entirely new domain from scratch — accounting, and reconciliations specifically. Reconciliations are the process accountants use to verify that financial records match across sources at the close of a period. I had no background in accounting coming in, and getting up to speed quickly enough to make good product and technical decisions was as much a part of the job as the engineering itself.</p>
        <p>I shipped 3 features to production for thousands of customers and took on ongoing ownership of the entire reconciliations product area. Within two months of joining, the engineer leading reconciliations departed mid-project, and I stepped in as the de facto technical owner — the only engineer actively working on one of the most critical parts of the product. The flagship initiative I took on was Custom-Month Reconciliations, which I designed and shipped from scratch. The original codebase was hardcoded to perform reconciliations only from the first to the last day of a calendar month. I extended this across the backend — reworking data structures and calculation logic throughout — to support any combination of months over a year, enabling close cadences like quarterly (March, June, September, December) or annual. I started with product scoping — user flow diagrams, product descriptions, and internal FAQs — then validated proposed business logic with accounting stakeholders before moving into technical scoping, breaking the work into milestones with effort estimates and verification criteria. I led a full round of QA testing to verify the feature end-to-end, set up a feature flag for the rollout, authored a finalized product description distributed to non-technical stakeholders, and delivered an internal presentation to the customer-facing support team before driving it to production.</p>
        <p>In parallel I inherited the Reconciliation Editor — an improved UX replacement for what was currently in production — and worked through an extensive debugging and fix process across frontend and backend logic, led QA testing, and drove it to production.</p>
    ...
    e5: {
      htmlShort: `
        ...
        <ul style="font-size:0.9rem;line-height:1.8;color:var(--muted);padding-left:1.2rem;margin-bottom:1.1rem">
          ...
          <li>Implemented AG-Grid (Angular data grid) enabling customizable user operations on the application's PostgreSQL database</li>
          <li>Collaborated with a teammate to extend the CP-SAT decision model with time-dependent logic, allowing optimal technology selections to persist across multi-year planning horizons; leveraged the enhanced model to present case studies to 25 Boeing engineers at a sustainability workshop in Seattle</li>
      ...
      `,
      html: `
        <p>Aurora Flight Sciences is structured as an R&D company, meaning engineers rotate across programs rather than owning a single product long-term. It was a fantastic place to start my career — I benefited enormously from their mentorship program and got to learn from principal engineers across the business, from software to mechanical to aerospace. In my time there I worked across six programs, and was promoted from Associate to Autonomy Engineer in eight months — a track that typically takes two or more years.</p>
        ...
        <div class="modal-timeline">
          ...
          <div class="modal-tl-item">
            ...
            <p>My first few months at Aurora were mostly an onboarding period — small bug fixes on the internal autonomy framework while my background check cleared before moving onto the DoD program.</p>
          </div>
          ...
          <div class="modal-tl-item">
            ...
            <p>The team's objectives were unclear and scope was undefined, which was creating real delivery risk. I put together a full program plan — defining deliverables, schedule, and scope for all five members — which program leadership approved with minor edits. The program ended before it could be realized, but it was a useful lesson in how quickly an unclear problem statement can stall a team.</p>
          </div>
          ...
          <div class="modal-tl-item">
            ...
            <span class="modal-tl-role">Paradigm — Business Insights Optimization Tool for Sustainable Aviation</span>
            <p>My last and longest program was Paradigm — a Boeing-led initiative to build a data analysis platform and CP-SAT (Constraint Programming - Satisfiability) optimization framework for informing Boeing's technology selection for their next-generation sustainable aircraft. The core optimizer uses Google OR-Tools CP-SAT to discretely optimize a global aviation network across multiple objective functions, balancing environmental and economic goals. The codebase had never been documented and was too complex to onboard new developers without it.</p>
            ...
            <p>From there the focus shifted to building out the platform. Starting in the existing Python/Dash stack, I built visualization and mapping tools that processed 2GB of data across multiple file formats. It became clear that Dash wasn't a viable path to hosting the application on Boeing's internal servers, so the decision was made to rebuild from scratch using modern full-stack technologies. I designed a custom PostgreSQL schema, migrated all the data into it, and built a React TypeScript frontend connected to a Python FastAPI backend — replicating all existing functionality including data selection via an AG-Grid (Angular data grid) implementation enabling customizable database operations, route and airport mapping, and interactive charts, while laying the groundwork for scalable internal deployment.</p>
      ...
      `,
      tags: ['Python', 'React', 'TypeScript', 'FastAPI', 'PostgreSQL', 'C++', 'Optimization', 'Full-Stack Development']
    },
    e4: {
      htmlShort: `
        <ul style="font-size:0.9rem;line-height:1.8;color:var(--muted);padding-left:1.2rem">
    ...
    e3: {
      htmlShort: `
        <ul style="font-size:0.9rem;line-height:1.8;color:var(--muted);padding-left:1.2rem">
          <li>Identified that <a href="https://open-dynamic-robot-initiative.github.io/" target="_blank">Open Dynamic Robot Initiative's (ODRI)</a> specialized PCBs (printed circuit boards) were the core driver of cost and capability constraints; proposed and led a full electrical redesign — reworking circuitry, computing, communication, and motor control systems — quadrupling communication capabilities and motor control bandwidth while reducing costs by over 91%</li>
          <li>Built out the technical documentation foundation for external collaborators: cost analysis, wiring schematics, circuit tutorials, and a bill of materials on <a href="https://github.com/EmiliaPsacharopoulos/open_robot_actuator_hardware" target="_blank">GitHub</a></li>
    ...
    e2: {
      ...
      `,
      html: `
        <p>I joined the Navy's Unmanned Maritime Systems Program Office (PMS 406) after my freshman year of college — the internship was originally intended to be in-person at the Navy Yard but moved remote due to COVID. My primary work was independently synthesizing hundreds of testing documents into executive summaries conveying key lessons learned, which went directly to Navy leadership within PMS 406. I also collaborated with senior engineers to organize a multi-day leadership conference for UUV (Unmanned Undersea Vehicle) Homeport program managers. The Strategic Capabilities Office recognized my communication and documentation work informally at the end of the summer.</p>
        <p>PMS 406 liked my work enough to offer a part-time continuation into my sophomore year, which I accepted. The highlight of the internship was getting to visit in person — touring Carderock (the Navy's ship and submarine testing facility) and JHU APL (Johns Hopkins University Applied Physics Laboratory) and seeing autonomous drone and boat technologies up close for the first time.</p>
      `,
